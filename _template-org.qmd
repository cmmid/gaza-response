...
params$cohort_ever_enrolled <- max(data_filtered$overall$cohort_id_enrolled,
                                   na.rm = TRUE)
# Data notes to accompany plots
data_notes <- list(
  current_summary = c('Median and 25-75% range of participant weight in kilograms (kg) and [BMI (body mass index)](https://www.cdc.gov/bmi/adult-calculator/index.html?), with % change from pre-October 2023. Negative % indicate weight loss.'),
  participation = c('Daily data availability: showing
...
- Previously enrolled participants without new data ("lost to follow up")'),
  note_change = c('Trends over time in participant current measurements, with comparison to pre-war levels (% change since October 2023), since the beginning of the project.'),
  note_bmi_category = c('Trends over time of proportion of staff with BMI in different [WHO categories](https://www.who.int/data/gho/data/themes/topics/topic-details/GHO/body-mass-index): underweight <18.5, normal 18.50 – 24.99, overweight ≥ 25, obese ≥ 30. Calculated as the number of measurements per week that fall in each category, divided by the total number of measurements that week ("N").')
)
```

## COL {.tabset width=30%}

### Current summary {{< bi info-fill label="About the data">}}

::: {.card .current_summary_card .lshtm_widget fill="false"}
...
::: {.card .participants_card .lshtm_widget fill="true"}
